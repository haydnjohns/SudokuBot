...
class SudokuRenderer:
    ...
    def __init__(self,
        ...
        if use_mnist:
            # Lazy loading of MNIST data - load only when needed first time
            self._mnist_digits = None
        else:
            self._mnist_digits = {i: [] for i in range(10)} # Empty dict if not used

    @property
    def mnist_digits(self):
        """Loads MNIST digits on first access if use_mnist is True."""
        if self.use_mnist and self._mnist_digits is None:
            self._mnist_digits = _load_mnist_digits()
        return self._mnist_digits


    def _get_random_digit_image(self, digit):
        """Selects a random image for the given digit (1-9) from available sources."""
        sources = []
        # Access MNIST digits via the property to trigger loading if needed
        ...
        if not sources:
             if not self.font_faces: self.font_faces = [cv2.FONT_HERSHEY_SIMPLEX]
             source = "font"
        else:
    ...
    def render_sudoku(self, grid_spec=None, allow_empty=True):
        ...
        if grid_spec is None:
            ...
            ground_truth_grid = np.array([[d if d is not None and d != 0 else 0 for d in row] for row in grid_spec], dtype=int)
        ...
        for i in range(GRID_SIZE + 1):
            thickness = random.randint(min_line, max_line -1) if i % 3 != 0 else random.randint(max(min_line, max_line -2), max_line)
        ...
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                ...
                target_w = int(self.cell_draw_size * scale)
                ...
                if source == "mnist" and digit_img is not None:
                    ...
                    rotated_digit = cv2.warpAffine(resized_digit, M, (target_w, target_h), borderMode=cv2.BORDER_CONSTANT, borderValue=(255, 255, 255))
                    ...
                    if mask.shape[0] == roi_h and mask.shape[1] == roi_w: # Ensure mask matches ROI after potential clipping
                    ...
                    # else: print warning or skip blending if sizes mismatch

                elif source == "font":
                    font_face = random.choice(self.font_faces)
                    digit_str = str(digit)
                    font_thickness = random.randint(1, 3)
                    # Estimate font scale (might need adjustment)
                    ...
                    # Skipping rotation for font text for simplicity
        ...
        shifted_corners = np.array([
            [random.uniform(0, max_shift_x * 0.8), random.uniform(0, max_shift_y)],
            [w - 1 - random.uniform(0, max_shift_x * 0.8), random.uniform(0, max_shift_y)],
            [w - 1 - random.uniform(-max_shift_x * 0.2, max_shift_x), h - 1 - random.uniform(0, max_shift_y * 0.5)],
            [random.uniform(-max_shift_x * 0.2, max_shift_x), h - 1 - random.uniform(0, max_shift_y * 0.5)]
        ], dtype="float32")
        min_dist = w * 0.1
...
def generate_and_save_test_example(filename_prefix="epoch_test_sudoku", force_regenerate=False):
    """Generates a fixed Sudoku image and its ground truth for callback testing."""
    ...
    if not force_regenerate and img_path.exists() and gt_path.exists():
        ...
        try:
            ...
            if ground_truth_grid.shape == (GRID_SIZE, GRID_SIZE):
                ...
                print(f"Existing ground truth file {gt_path} has wrong shape {ground_truth_grid.shape}. Regenerating.")
        except Exception as e:
            print(f"Error loading ground truth {gt_path}: {e}. Regenerating.")

    print(f"Generating new test example: {img_path}, {gt_path}")
    # Define a reasonably complex, fixed grid
    test_grid_spec = [
    ...
    renderer = SudokuRenderer() # Use default settings for consistency
...
if __name__ == "__main__":
    ...
    # Ensure the test example exists for potential use by classifier training
    try:
        generate_and_save_test_example()
    except Exception as e:
        print(f"[ERROR] Could not generate/save test example: {e}")
